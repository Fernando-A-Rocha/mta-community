            ...
            <script>
                ...
                function updateUploadMode() {
                    ...
                    if (imagesField) {
                        ...
                        // Also hide images info if images field is hidden
                        if (imagesInfo && imagesField.classList.contains('hidden')) {
                ...
                const zipFileInfo = document.getElementById('zip_file_info');
                const zipFileName = document.getElementById('zip_file_name');

                // Store file info when file is selected
                // Use event delegation on the form to catch file input changes
                const form = document.getElementById('resource-upload-form');
                if (form) {
                    form.addEventListener('change', function(e) {
                        // Check if this is the zip file input
                        let actualInput = e.target;
                        if (actualInput.tagName !== 'INPUT' || actualInput.type !== 'file' || actualInput.name !== 'zip_file') {
                            return;
                        }

                        const file = actualInput.files?.[0];
                        if (file && zipFileInfo && zipFileName) {
